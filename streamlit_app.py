...
# ─── UTILS & TIMEZONE CONFIGURATION ───────────────────────────────────────────
def now_ist():
    """Returns current time in India Standard Time (IST)"""
    return datetime.now(timezone(timedelta(hours=5, minutes=30)))

# SQL snippet to calculate current timestamp in IST within Snowflake queries
IST_SQL = "CONVERT_TIMEZONE('UTC', 'Asia/Kolkata', CURRENT_TIMESTAMP())"
...
def get_snowflake_session():
    ...
    try:
        ...
        connection_parameters = {
            "account": os.getenv("SNOWFLAKE_ACCOUNT", "ETUFWEP-MCB24323"),
            "user": os.getenv("SNOWFLAKE_USER", "ACTESTUSER"),
            "password": os.getenv("SNOWFLAKE_PASSWORD", "Cloudlabs@12345"),
...
def hash_password(password):
    return session.sql("SELECT SHA2(?) AS H", params=[password]).to_pandas().iloc[0]["H"]
...
def authenticate(username, password):
    ...
    result = session.sql(
        "SELECT USER_ID, USERNAME, ROLE, EMAIL, MOBILE, IS_TEMP_PASSWORD, IS_ACTIVE FROM INVOICE_MGMT.PUBLIC.ADMIN_USERS WHERE USERNAME = ? AND PASSWORD_HASH = ? AND IS_ACTIVE = TRUE",
    ...
    result = session.sql(
        "SELECT CONSULTANT_ID, USERNAME, NAME AS FULL_NAME, EMAIL, MOBILE, IS_TEMP_PASSWORD, IS_ACTIVE FROM INVOICE_MGMT.PUBLIC.CONSULTANTS WHERE USERNAME = ? AND PASSWORD_HASH = ? AND IS_ACTIVE = TRUE",
        params=[username, pwd_hash],
    ).to_pandas()
    if len(result) > 0:
        row = result.iloc[0].to_dict()
...
def forgot_password_verify(username, email):
    # Check ADMIN_USERS
    result = session.sql(
        "SELECT USER_ID, EMAIL, 'ADMIN' AS AUTH_SOURCE FROM INVOICE_MGMT.PUBLIC.ADMIN_USERS WHERE USERNAME = ? AND EMAIL = ? AND IS_ACTIVE = TRUE",
    ...
    # Check CONSULTANTS
    result = session.sql(
        "SELECT CONSULTANT_ID, EMAIL, 'CONSULTANT' AS AUTH_SOURCE FROM INVOICE_MGMT.PUBLIC.CONSULTANTS WHERE USERNAME = ? AND EMAIL = ? AND IS_ACTIVE = TRUE",
        params=[username, email],
    ).to_pandas()
    if len(result) > 0:
            ...
            with st.form("consultant_change_pwd_form"):
                ...
                if st.form_submit_button("Update Password", type="primary", use_container_width=True):
                    if not cur_pwd or not new_pwd or not confirm_pwd:
                        ...
                        verify = session.sql(
                            "SELECT CONSULTANT_ID FROM INVOICE_MGMT.PUBLIC.CONSULTANTS WHERE CONSULTANT_ID = ? AND PASSWORD_HASH = ?",
...
def legal_admin_portal():
            ...
            with st.form("admin_change_pwd_form_legal"):
                ...
                if st.form_submit_button("Update Password", type="primary", use_container_width=True):
                    if not cur_pwd or not new_pwd or not confirm_pwd:
                        ...
                        cur_hash = hash_password(cur_pwd)
                        verify = session.sql(
                            "SELECT USER_ID FROM INVOICE_MGMT.PUBLIC.ADMIN_USERS WHERE USER_ID = ? AND PASSWORD_HASH = ?",
    ...
    # ─── METRICS ─────────────────────────────────────────────────────────────
    total_consultants = session.sql("SELECT COUNT(*) AS CNT FROM INVOICE_MGMT.PUBLIC.CONSULTANTS").to_pandas().iloc[0]["CNT"]
    active_consultants = session.sql("SELECT COUNT(*) AS CNT FROM INVOICE_MGMT.PUBLIC.CONSULTANTS WHERE IS_ACTIVE = TRUE").to_pandas().iloc[0]["CNT"]
            ...
            with st.form("create_consultant_form"):
                ...
                if submitted:
                    ...
                    if errors:
                        ...
                        existing = session.sql(
                            "SELECT CONSULTANT_ID FROM INVOICE_MGMT.PUBLIC.CONSULTANTS WHERE CONSULTANT_ID = ?",
                            params=[c_id.upper()],
                        ).to_pandas()
                        if len(existing) > 0:
                            ...
                            pwd_hash = session.sql("SELECT SHA2(?) AS H", params=[temp_pwd]).to_pandas().iloc[0]["H"]
                        ...
                        if not has_errors:
                            ...
                            if st.button("Create All Consultants", type="primary", use_container_width=True):
                                ...
                                for _, row in df.iterrows():
                                    ...
                                    ex = session.sql("SELECT CONSULTANT_ID FROM INVOICE_MGMT.PUBLIC.CONSULTANTS WHERE CONSULTANT_ID = ?", params=[cid]).collect()
                                    if len(ex) > 0:
                                        skipped += 1
                                        continue
